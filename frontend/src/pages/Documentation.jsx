          ...
          <aside className="hidden w-52 shrink-0 lg:block">
            <div className="sticky top-24">
              ...
              <div className="space-y-1">
                ...
                <Link to="/qna" className="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
                  <MessageCircleQuestion className="h-3.5 w-3.5" /> Q&amp;A
                </Link>
                <Link to="/submit" className="flex items-center gap-1.5 rounded-md px-3 py-1.5 text-sm text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
          ...
          <main className="min-w-0 flex-1">
            ...
            <section id="submission" className="scroll-mt-24">
              ...
              <ol className="space-y-6">
                {[
                  {
                    step: '1',
                    title: 'Go to Submit Tournament',
                    desc: 'Click the "Submit Tournament" button in the navigation bar, or visit /submit directly.',
                  },
                  {
                    step: '2',
                    title: 'Fill in tournament details',
                    desc: 'Complete all required fields — name, format, dates, prize pool, max teams, and registration link. See the Field Reference below for notes on each field.',
                  },
                  {
                    step: '3',
                    title: 'Add organiser info',
                    desc: 'Provide your organiser name, a contact method (WhatsApp or email), and the email address where you want to receive the approval notification.',
                  },
                  {
                    step: '4',
                    title: '(Optional) Upload a banner',
                    desc: 'Upload a landscape banner image. Recommended: 1280×720 px (16:9), JPG or PNG, max 5 MB.',
                  },
                  {
                    step: '5',
                    title: 'Submit for review',
                    desc: "Your submission enters the pending queue. An admin will review it within 1–2 business days and you'll receive an email once a decision is made.",
                  },
                ].map((item) => (
                ...
                {[
                  {
                    status: 'Pending',
                    color: 'pending',
                    desc: 'Your submission is in the review queue. Typically reviewed within 1–2 business days. No action needed from you.',
                  },
                  {
                    status: 'Approved',
                    color: 'current',
                    desc: "Your tournament is now live and publicly visible on Esportorium. You'll receive a notification at the organiser email you provided.",
                  },
                  {
                    status: 'Rejected',
                    color: 'past',
                    desc: "Your submission didn't meet the listing criteria. The notification email will include the reason. You may resubmit after addressing the issue.",
                  },
                ].map((item) => (
            ...
            <section id="images" className="scroll-mt-24">
              ...
              <p className="mt-3 text-xs text-muted-foreground">
                Banners are stored on Cloudflare R2 and served via CDN. Portrait or square images will be displayed with letterboxing.
              </p>
            </section>
                    ...
                    {[
                      ...
                      <tr key={field} className="hover:bg-muted/40">
                        ...
                        <td className="px-4 py-3">
                          <Badge
                            variant={required === 'Yes' ? 'default' : required === 'No' ? 'secondary' : 'outline'}
                            className="text-xs"
                          >
                            {required}
                          </Badge>
