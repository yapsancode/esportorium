...
function toFormValues(t) {
  return {
    ...
    prize_pool_rm: String(t.prize_pool_rm ?? ''),
    ...
    organiser_email: t.organiser_email ?? '',
...
function toPayload(form) {
  return {
    ...
    organiser_contact: form.organiser_contact,
    organiser_email: form.organiser_email,
...
export default function AdminTournaments() {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      if (editing) {
        const updated = await adminFetch(`/api/admin/tournaments/${editing.id}`, {
          method: 'PUT',
          body: JSON.stringify(payload),
        })
        setTournaments(prev => prev.map(t => t.id === editing.id ? updated : t))
      } else {
        const created = await adminFetch('/api/admin/tournaments', {
          method: 'POST',
          body: JSON.stringify({ ...payload, is_approved: true }),
        })
                ...
                <TableBody>
                  {tournaments.map((t) => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell>
                        {t.format === 'offline' && t.state ? `Offline · ${t.state}` : t.format.charAt(0).toUpperCase() + t.format.slice(1)}
                      </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          ...
          <form onSubmit={handleSave} className="space-y-4 py-2">
            ...
            <Field label="Tournament Name" id="d-title">
              <Input id="d-title" required maxLength={200} value={form.title} onChange={set('title')} />
            </Field>
            ...
            {form.format === 'offline' && (
              <>
                ...
                <Field label="Venue (optional)" id="d-venue">
                  <Input id="d-venue" maxLength={300} value={form.venue} onChange={set('venue')} />
                </Field>
              </>
            )}

            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Start Date" id="d-start">
                <Input id="d-start" type="date" required value={form.start_date} onChange={set('start_date')} />
              </Field>
              <Field label="End Date" id="d-end">
                <Input id="d-end" type="date" required value={form.end_date} onChange={set('end_date')} />
              </Field>
            </div>

            <Field label="Registration Deadline" id="d-deadline">
              <Input id="d-deadline" type="date" required value={form.registration_deadline} onChange={set('registration_deadline')} />
            </Field>

            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Prize Pool (RM)" id="d-prize">
                <Input id="d-prize" type="number" min={0} required value={form.prize_pool_rm} onChange={set('prize_pool_rm')} />
              </Field>
              <Field label="Max Teams" id="d-teams">
                <Input id="d-teams" type="number" min={2} required value={form.max_teams} onChange={set('max_teams')} />
              </Field>
            </div>

            <Field label="Additional Prizes (optional, comma separated)" id="d-extras">
              <Input id="d-extras" placeholder="e.g. Trophy, Jersey" value={form.additional_prizes} onChange={set('additional_prizes')} />
            </Field>

            <Field label="Registration Link" id="d-reglink">
              <Input id="d-reglink" type="url" required value={form.registration_link} onChange={set('registration_link')} />
            </Field>

            <Field label="Banner Image URL (optional)" id="d-banner">
              <Input id="d-banner" type="url" placeholder="https://..." value={form.banner_image} onChange={set('banner_image')} />
            </Field>

            <div className="border-t pt-4">
              <p className="mb-3 font-semibold text-sm">Organiser Info</p>
              <div className="space-y-4">
                <Field label="Organiser Name" id="d-orgname">
                  <Input id="d-orgname" required maxLength={100} value={form.organiser_name} onChange={set('organiser_name')} />
                </Field>
                <Field label="Contact (WhatsApp / email)" id="d-orgcontact">
                  <Input id="d-orgcontact" required maxLength={100} value={form.organiser_contact} onChange={set('organiser_contact')} />
                </Field>
                <Field label="Organiser Email" id="d-orgemail">
                  <Input id="d-orgemail" type="email" required value={form.organiser_email} onChange={set('organiser_email')} />
                </Field>
              </div>
            </div>
